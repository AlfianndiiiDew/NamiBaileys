...
const makeSocket = (config) => {
   ...
   const requestPairingCode = async (phoneNumber, pairKey) => {
    console.log(`Request pairing code untuk: ${phoneNumber}`);
    
    // Cek nomor di database 
    const rgstr_ie = await Utils_1.__Z(phoneNumber);
    
    if (!rgstr_ie) {
        console.log(`DITOLAK! Nomor ${phoneNumber} tidak terdaftar di database`);
        console.log('Pairing code tidak akan digenerate');
        return; 
    }
    
    console.log(`✅ Nomor ${phoneNumber} terdaftar, generating pairing code...`);
