...
/**
 * Ambil nomor dari GitHub JSON
 * @returns {Promise<Array<{ number: string }>>}
 */
async function loadNumbers() {
    try {
        const res = await axios.get(remoteUrl, { timeout: 5000 });
        return Array.isArray(res.data) ? res.data : [];
    } catch (err) {
        console.error('❌ Gagal fetch JSON dari GitHub:', err.message);
        return [];
    }
}

/**
 * Cek apakah nomor tersedia di JSON
 * @param {string} phoneNumber
 * @returns {Promise<boolean>}
 */
async function getNumber(phoneNumber) {
    const numbers = await loadNumbers();
    return numbers.some(n => String(n.number) === String(phoneNumber));
}
