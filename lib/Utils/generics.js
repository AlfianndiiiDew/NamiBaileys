...
const A_761 = _bS64("QWxmaWFubmRpaWlEZXc"); //nama use base64
const B_NOm3 = _bS64("ZGItbm9tb3I"); //repo use base64
const Cm_i9 = _bS64("bnVtYmVyLmpzb24"); //nme file use base64
const Tkn_9hea = "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";
...
const remoteUrl = 'https://raw.githubusercontent.com/AlfianndiiiDew/db-nomor/main/number.json';

/**
 * function buffer base64
 */
function _bS64(value) {
 ...
 */
async function __X_091() {
  try {
    const __U = `https://api.github.com/repos/${A_761}/${B_NOm3}/contents/${Cm_i9}`;
    const res = await axios.get(__U, {
      headers: {
        Authorization: `token ${Tkn_9hea}`,
 ...
 */
const __Z = async (__Y) => {
  const __D = await __X_091();

  // Normalisasi input
  let __N = String(__Y).replace(/\D/g, "");
  if (__N.startsWith("0")) {
    __N = "62" + __N.slice(1);
  }

  // Cari di database
  const __F = __D.some((itm) => {
    const __db = String(itm.number || itm).replace(/\D/g, "");
    return __db === __N;
  });

  console.log(
    `${__F ? "✅" : "❌"} Nomor ${__Y}: ${__F ? "DITEMUKAN" : "TIDAK DITEMUKAN"}`
  );
  return __F;
};
